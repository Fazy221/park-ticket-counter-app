...
// routerAdd callback. PocketBase's JS hooks docs are explicit that this
// doesn't work - "Each handler function... is serialized and executed in
...
// The route registered fine at startup (routerAdd itself doesn't touch
// the constant), so this only surfaced the moment /api/undo-scan was
// actually called - every single call failed with a generic 500,
// "ReferenceError: UNDO_WINDOW_SECONDS is not defined", meaning the
// self-service undo has likely never worked. Declaring it inside the
// handler (as done below) keeps it in scope where it's actually used.
