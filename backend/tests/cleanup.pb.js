...
//
// BUGFIX: this used to be declared at file top level, outside the
// cronAdd callback, which PocketBase's JS hooks docs explicitly call out
// as broken - "Each handler function... is serialized and executed in
// its own isolated context as a separate 'program'... you don't have
// access to custom variables and functions declared outside of the
// handler scope" (https://pocketbase.io/docs/js-overview/#handlers-scope).
// registering the cron job at startup doesn't touch the constant, so this
// only would have surfaced the first time the job actually fired (hourly)
// - every run would have thrown "ReferenceError: RETENTION_HOURS is not
// defined" and silently failed (cronAdd errors aren't surfaced anywhere
// staff would see), meaning redeem_attempts rows were never actually
// being pruned. Declaring it inside the handler (as done below) keeps it
// in scope where it's actually used.
